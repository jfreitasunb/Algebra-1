...
\[
    x R y \mbox{ quando } x\mid y \mbox{ e } y\mid x.
\]
...
\questao Seja $A$ um conjunto n\~ao vazio. Suponha que $R_1$ e $R_2$ s\~ao rela\c{c}\~oes de equival\^encias sobre $A$. Defina
\[
    R = \{(x,y) \in A\times A \mid (x,y) \in R_1 \mbox{ e } (x,y) \in R_2\}.
\]
Mostre que:
\begin{enumerate}[label={\alph*})]
    \item $R$ \'e uma rela\c{c}\~ao de equival\^encia sobre $A$.
    \item Se $c_R(x)$, $c_{R_1}(x)$ e $c_{R_2}(x)$ denota a classe de equival\^encia de $x$ com respeito a $R$, $R_1$ e $R_2$, mostre que $c_R(a) = c_{R_1}(a) \cap c_{R_2}(a)$.
\end{enumerate}
\end{document}