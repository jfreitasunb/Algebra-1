...
{\Large\bf \disciplina\ - Turma \turma\ -- \semestre$^{o}$/\ano} \\ \vspace{9pt} {\large\bf
  $4^{\underline{a}}$ Lista de Exerc{\'\i}cios -- Funções}\\ \vspace{9pt} Prof. Jos{\'e} Ant{\^o}nio O. Freitas
\end{center}
\hrule
...
\begin{multicols}{2}
	\begin{enumerate}[label={\roman*})]
		...
		\item Em $\z_m$ vamos denotar a operação de soma $\oplus$ por $+$.
		\item Em $\z_m$ vamos denotar a operação de multiplicação $\otimes$ por $\cdot$.
	\end{enumerate}
\end{multicols}
...
\questao Mostre que a função $h : \real^2 \to \real^2$ tal que $h(x, y) = (\sqrt[3]{x}, y^5)$ é sobrejetora.
\vspace{.3cm}

\questao Considere a fun{\c c}{\~a}o $g : \z_5 \times \z_9 \to \z_5 \times \z_9$ tal que $f(\overline{x},\overline{y}) = (\overline{2} \overline{x} + \overline{3}, 4\overline{y} + \overline{5})$.
\begin{enumerate}[label={\alph*})]
	\item $g$ é injetora?
...
\questao Considere a fun{\c c}{\~a}o $g : \z_5 \times \z_7 \to \z_5 \times \z_7$ tal que $f(\overline{x},\overline{y}) = (\overline{x} + \overline{3}, \overline{6}\overline{y} + \overline{2})$. Determine a função inversa de $g$.
...
\questao Dois conjuntos, $A$ e $B$, são \textbf{equipotentes} quando $A = B = \emptyset$ ou existe $f : A \to B$ função	bijetora. Mostre, em cada caso, que os conjuntos $A$ e $B$ são equipotentes, encontrado uma função bijetora $f : A \to B$ em cada caso:
\begin{enumerate}[label={\alph*})]
	\item $A = \n$ e $B = \n^*$
	\item $A = \z$ e $B = \n$
	\item $A = \real$ e $B = \real$
	\item $A = ]-1, 1[$ e $B = ]c,d[$ com $c$ e $d$ números reais e $c < d$.
\end{enumerate}
