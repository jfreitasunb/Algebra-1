...
\begin{prova}
    ...
    \begin{enumerate}[label={\roman*})]
        \item Se $B$ é um subanel de $A$, então $x + (-y) \in B$, $x\cdot y \in B$ para todos $x$, $y \in B$.

        \item Se $x + (-y) \in B$, $x\cdot y \in B$ para todos $x$, $y \in B$, então $B$ é um subanel de $A$.
    \end{enumerate}

    A prova de \textit{(i)} é uma consequência direta da definição de subanel.

    Agora para prova \textit{(ii)}, vamos mostrar que $(B, +, \cdot)$ é um anel. Inicialmente obserque que na definição de anel, Definição \ref{definicaoanel}, os itens \ref{associatividadesoma}
\end{prova}
