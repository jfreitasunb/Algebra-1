...
\begin{definicao}
    Seja $A$ um conjunto n{\~a}o vazio. Dizemos que $A$ est{\'a} munido (ou equipado) de uma \textbf{opera{\c c}{\~a}o bin{\'a}ria} quando
    existe uma fun{\c c}{\~a}o
	\begin{align*}
		&\Delta : A \times A \to A\\
...
\begin{exemplos}
	\begin{enumerate}[label={\arabic*})]
        ...
        \item Seja $m > 1$, $m \in \z$ fixo. A soma \eqref{soma_modulo_m} e a multiplica\c{c}\~ao \eqref{multiplicacao_modulo_m} definidos em $\z_m =
            \{\overline{0},\overline{1},...,\overline{m-1}\}$ s\~ao uma opera\c{c}\~oes bin\'arias.
...
\begin{definicao}\label{definicaoanel}
    Seja $A$ um conjunto n{\~a}o vazio no qual est\~ao definidas duas opera{\c c}{\~o}es bin\'arias $\oplus$ e $\otimes$, chamadas
    \textit{soma} e \textit{produto}.  Dizemos que $(A, \oplus, \otimes)$ {\'e} um \textbf{anel} quando as seguintes condi{\c c}{\~o}es s{\~a}o verdadeiras:
    ...
    propriedade {\'e} chamada \textbf{propriedade associativa} da soma.\label{associatividadesoma}
...
\end{observacoes}
=======
        \item Seja $(A, \oplus, \otimes)$ um anel. Quando n\~ao houver chance de confus\~ao com rela\c{c}\~ao \`as opera\c{c}\~oes envolvidas diremos simplesmente que $A$ \'e um
            anel.
    \end{enumerate} \end{observacoes}

>>>>>>> b6a19fa3830223065f7b18515e0c561476e35e91
\begin{exemplos}
	\begin{enumerate}[label={\arabic*})]
...
\begin{proposicao}\label{proposicao_subanel}
    Seja $(A, +,\cdot)$ um anel. Um subconjunto n{\~a}o vazio $B\subseteq A$ {\'e} um subanel de $A$ se, e somente se, $x + (-y) \in B$ e $x\cdot y \in B$ para todos $x$, $y \in
    B$.
\end{proposicao}
\begin{prova}
