...
\begin{definicao}
	Diz-se que o \textbf{valor lógico} de uma proposição é ``verdade'' (V) se a proposição é verdadeira e ou ``falsidade'' (F) se a proposição é falsa.
\end{definicao}

\section{Príncipio da não contradição e do 3$^o$ excluído} % (fold)
\label{sec:príncipio_da_não_contradição_e_do_3}
\begin{enumerate}
...
\begin{enumerate}
	\item Se $\mathcal{H}$, então $\mathcal{T}$.
	...
	\item $\mathcal{H}$ se, e somente se, $\mathcal{T}$ ou $\mathcal{H}$ se, e só se, $\mathcal{T}$.

	Esse tipo de proposição será decomposta em duas proposições no formato anterior. Isto é:
	\begin{enumerate}
		\item Se $\mathcal{H}$, então $\mathcal{T}$.
		\item Se $\mathcal{T}$, então $\mathcal{H}$.
	\end{enumerate}
...
\end{proposicao}
\begin{prova}
	Segue da definição de diferença de conjuntos.
\end{prova}
